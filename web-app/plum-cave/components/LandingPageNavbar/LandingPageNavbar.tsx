...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const [initialHighlightSet, setInitialHighlightSet] = useState(false);

  useEffect(() => {
    if (!initialHighlightSet) {
        setActiveSection(sections[0].id);
        setInitialHighlightSet(true);
    }
  }, [initialHighlightSet, sections]);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      switch (i18n.language) {
        case 'he':
          setIsSectionHighlightVisible(width >= 800);
          break;
        case 'en':
          setIsSectionHighlightVisible(width >= 712);
          break;
        case 'es_ar':
          setIsSectionHighlightVisible(width >= 864);
          break;
        default:
          setIsSectionHighlightVisible(false);
      }
    };
  ...
  }, [sections, isScrolling, isRTL]);

  useEffect(() => {
    const activeTab = document.querySelector<HTMLElement>(`#nav-${activeSection}`);
    if (activeTab && highlightRef.current && initialHighlightSet) {
  ...
  }, [activeSection, disableGradualHighlight, isRTL, initialHighlightSet]);
  ...
  return (
    <StickyNavbarWrapper>
      <NavbarContainer desktopPadding={desktopPadding} mobilePadding={mobilePadding} desktopFont={desktopFont} mobileFont={mobileFont} desktopSubFont={desktopSubFont} mobileSubFont={mobileSubFont} maxWidth="1200px" style={{ transform: isRTL ? 'scaleX(-1)' : 'none' }} >
        {/* Left Section */}
        <LeftSection style={{ transform: isRTL ? 'scaleX(-1)' : 'none' }}>
          <AppNameContainer isRTL={isRTL}>
            <a href="/" style={{ textDecoration: 'none' }}>
              <AppName desktopFont={desktopFont} mobileFont={mobileFont} themeColor="white" foregroundHoverColor="var(--first-theme-color)" isRTL={isRTL} >
                {appName}
              </AppName>
            </a>
          </AppNameContainer>
          <SubInscriptionContainer>
            <SubInscription desktopFont={desktopSubFont} mobileFont={mobileSubFont} themeColor="var(--foreground)">
              {appSubInscription}
            </SubInscription>
          </SubInscriptionContainer>
        </LeftSection>
          ...
          <CenterSection>
            <TabsContainer>
              {sections.map((section) => (
                <Tab key={section.id} id={`nav-${section.id}`} onClick={() => handleSectionClick(section.id)} style={{ transform: isRTL ? 'scaleX(-1)' : 'none' }} >
                  {section.label}
                </Tab>
              ))}
              <Highlight ref={highlightRef} activeBackgroundGradient={activeBackgroundGradient} highlightBorderRadius={highlightBorderRadius} />
            </TabsContainer>
          </CenterSection>
        )}
        {/* Right Section */}
        ...
        {isRTL ? (
            <>
              <FancyNavbar
                items={[
                  { icon: <IconLogin2 width={26} />, label: t('log-in-the-verb') },
                ]}
