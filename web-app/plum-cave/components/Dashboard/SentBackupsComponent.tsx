...
interface SentBackupsComponentProps {
...
const SentBackupsComponent: React.FC<SentBackupsComponentProps> = ({ backups, onDeleteBackup, onDownloadBackup, onShareBackup }) => {
  ...
  return (
    <div className="sent-backups w-full">
          ...
          <HighlightEffect key={backup.id}>
            <div className="p-4 rounded-lg border border-[var(--lightened-background-adjacent-color)] bg-[var(--card-background)] relative">
              <div className={`space-y-2 ${isRTL ? 'text-right' : 'text-left'}`}>
                ...
                {renderFieldWithSpecificUnknown(t('recipient'), backup.recipient, t('unknown'))}
              </div>
      ...
      {showDeleteBackupModal && (
        <ConfirmationPopUp
          ...
          messageText={t('are-you-sure-you-want-to-delete-this-reference')}
          secondLineText={t('this-will-not-delete-the-backup-for-the-recipient')}
      ...
      {showDisplayTagModal && generatedTag && (
        <DisplayShareableTagForBackup shareableTag={generatedTag} onClose={() => setShowDisplayTagModal(false)} />
      )}
      {showGetRecipientEmailModal && (
        <GetRecipientEmail onClose={() => setShowGetRecipientEmailModal(false)} onSubmit={handleSubmitRecipientEmail} />
      )}
    </div>
  );
};

export default SentBackupsComponent;
