...
import { Backup } from '../types'; // Import the Backup type
import { evaluateBackupIntegrity, generateFileSizeString, formatDate, isRTLCheck } from "@/components/utils";
...
const DecryptModal: React.FC<DecryptModalProps> = ({
  ...
  const messages = externalMessages || evaluateBackupIntegrity(backup, t);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (modal) {
        const isOverflowing =
          modal.scrollHeight > modal.clientHeight || modal.scrollWidth > modal.clientWidth;
  
        const exceedsHeightThreshold =
          modal.scrollHeight > window.innerHeight * 0.9;
      ...
      <div className="file-processing-popup" style={{ zIndex: '1000001', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', overflowX: 'hidden', }}>
        <div ref={modalRef} className="modal-background" style={{ width: isFullScreen ? "100vw" : "min(90vw, 640px)", height: isFullScreen ? "100vh" : "auto", padding: "32px", display: "flex", flexDirection: "column", alignItems: "center", position: "relative", gap: "8px", overflowY: isFullScreen ? "auto" : undefined, }}>
          ...
          <div style={{ width: '100%', marginTop: '1rem' }}>
            <div className="flex" style={{ width: '100%', direction: isRTL ? 'rtl' : 'ltr' }}>
              <label className="text-[var(--foreground)] font-semibold" dir={isRTL ? 'rtl' : 'ltr'} style={{ textAlign: isRTL ? 'right' : 'left' }}>
                <span dir={isRTLCheck(t('size-of-encrypted-backup')) ? 'rtl' : 'ltr'}>{t('size-of-encrypted-backup')}:</span>
              </label>
              <span style={{ marginLeft: isRTL ? '0' : '8px', marginRight: isRTL ? '8px' : '0', color: 'var(--foreground)', direction: 'ltr' }}>{encryptedSize}</span>
            </div>
          </div>
          ...
          {downloads !== t('unknown-downloads') ? (
            <div style={{ width: '100%', marginTop: '1rem' }}>
              <div style={{ width: '100%', direction: isRTL ? 'rtl' : 'ltr' }}>
                <label className="text-[var(--foreground)] font-semibold" dir={isRTL ? 'rtl' : 'ltr'} style={{ textAlign: isRTL ? 'right' : 'left' }}>
                  <span dir={isRTLCheck(t('downloads')) ? 'rtl' : 'ltr'}>{t('downloads')}:</span>
                </label>
                <span style={{ marginLeft: isRTL ? '0' : '8px', marginRight: isRTL ? '8px' : '0', color: 'var(--foreground)' }}>{downloads}</span>
              </div>
            </div>
          ) : (
            <div style={{ width: '100%', marginTop: '1rem' }}>
              <div style={{ width: '100%', direction: isRTL ? 'rtl' : 'ltr' }}>
                <label className="text-[var(--foreground)] font-semibold" dir={isRTL ? 'rtl' : 'ltr'} style={{ textAlign: isRTL ? 'right' : 'left' }}>
                  <span dir={isRTLCheck(t('unknown-downloads')) ? 'rtl' : 'ltr'}>{t('unknown-downloads')}</span>
                </label>
              </div>
            </div>
          )}
          <div style={{ width: '100%', marginTop: '1rem' }}>
            <div style={{ width: '100%', direction: isRTL ? 'rtl' : 'ltr' }}>
              <label className="text-[var(--foreground)] font-semibold" dir={isRTL ? 'rtl' : 'ltr'} style={{ textAlign: isRTL ? 'right' : 'left' }}>
                <span dir={isRTLCheck(t(owner ? 'owner' : 'access')) ? 'rtl' : 'ltr'}>
                  {t(owner ? 'owner' : 'access')}:
                </span>
              </label>
              <span style={{ marginLeft: isRTL ? '0' : '8px', marginRight: isRTL ? '8px' : '0', color: 'var(--foreground)' }}>
                {owner || access} {/* Safe access */}
              </span>
            </div>
          </div>
          {messages.length > 0 && (
            <div style={{ width: '100%', marginTop: '1rem' }}>
              <div style={{ width: '100%', direction: isRTL ? 'rtl' : 'ltr' }}>
                <label className="text-[var(--foreground)] font-semibold" dir={isRTL ? 'rtl' : 'ltr'} style={{ textAlign: isRTL ? 'right' : 'left' }}>
                  <span dir={isRTLCheck(messagesListHeader) ? 'rtl' : 'ltr'}>{messagesListHeader}:</span>
                </label>
              </div>
              <div style={{ overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: '#151419 #151419', marginTop: '1rem' }}>
                {messages.map((message, index) => (
                  <div key={index} dir={isRTL ? 'rtl' : 'ltr'} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                    {isRTL ? (
                      <span style={{ margin: '4px', transform: 'scaleX(-1)' }}>
                        {message.success ? (
          ...
          <div style={{ marginTop: '8px' }} className={isMobile ? 'flex flex-col gap-6 w-full' : 'flex gap-6 w-full'}>
            {isRTL ? (
              <>
                <HalomotButton text={secondButtonText} onClick={() => onButtonClicked('second')} gradient={
                ...
                {displayFirstButton !== false && (
                  <HalomotButton text={firstButtonText} onClick={() => onButtonClicked('first')} gradient={
                    i18n.language === 'he' ? 'linear-gradient(to right, var(--second-theme-color), var(--first-theme-color))' : 'linear-gradient(to right, var(--first-theme-color), var(--second-theme-color))'
                  } fillWidth={isMobile} />
                )}
              </>
